...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  // get the current user to access old avatar url
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  if (!username?.trim()) {
    throw new ApiError(400, "Username Missing in URL")
  }

  // instead of finding one by one 
  // User.find({ username })


  // we use aggregation method (MONGODB Aggregation PipeLines)
  ...
  console.log("The value of channel is : ", channel)
...
const getWatchHistory = asyncHandler(async (req, res) => {
  // req.user._id dai xai string which is not MongoDb id format 
  // mongo db id format me ObjectId('id') hoixai
  const user = await User.aggregate([
    {
      $match: {
        // because of above reasons we use new mongoose.Types.ObjectId to get the whole id in MongoDb format
